...
<script src="{{ asset('site/js/bg.js') }}"></script>
...
<script>
    $(document).ready(function() {
        ...
        }, "{{ __('الملف يجب ان يكون بحجم اقل من او يساوي 5 ميجا') }}");
        ...
        $("#join_form").validate({
            ...
            submitHandler: function(form) {
                var formData = new FormData(form);
                let url = form.action;
                $('.ctm-join').prop('disabled', true);
                // Hide the button
                $('.ctm-join').hide();


                // Add a spinner
                $('.ctm-join').parent().append(
                    `<div class="spinner-border"  style="width: 3rem;height: 3rem;margin: auto;/* padding: 24px; */display: flex;"   role="status">
                <span class="sr-only">Loading...</span>
                </div>
                    `
